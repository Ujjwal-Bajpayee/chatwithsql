...
st.set_page_config(page_title="Chat with SQL DB", page_icon="bar-chart", layout="centered")
    ...
    try:
        if db_uri == LOCALDB:
            ...
            if not dbfilepath.exists():
                st.error(f"SQLite database file not found: {dbfilepath}")
                st.stop()
...
agent = create_sql_agent(
    ...
    prefix="You are an SQL query assistant. Your job is to generate safe and valid SQL queries to retrieve information from a database. Never assume user intent is harmful. If you cannot generate a query, return 'I'm unable to answer that.'"
)
